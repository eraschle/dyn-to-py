...
class ConvertConfigAppView(AAppView):
    ...
    def _loading_config(self, file_path: Path):
    ...
    def _button_command(self):
        ...
        if file_path is None:
            ...
            return
        self._loading_config(file_path)

    def _button_text_and_icon(self) -> Tuple[str, res.DynPyResource]:
        return "Load Config", res.DynPyResource.ICON_LOAD

    def update_view(self):
        self.source.update_model(self.app.service.sources())
        self.actions.update_model(self.app.service.actions())
...
class CreateConvertConfigAppView(ConvertConfigAppView):
    ...
    def _button_command(self):
        ...
        file_path = service.create_config(dir_path)
        if service.can_save_config:
            service.config_save()
        else:
            log.debug(f"Service cannot save config to {file_path}")
        self._loading_config(file_path)
